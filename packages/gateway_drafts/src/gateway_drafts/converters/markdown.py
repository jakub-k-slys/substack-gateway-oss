...
def markdown_to_draft_doc(markdown: str) -> dict[str, Any]:
    # Only expand literal ``\n`` escapes for single-line input. Multi-line
    # input already carries real newlines, and expanding there would corrupt
    # LaTeX/escape sequences that legitimately start with ``\n`` (``\neg``,
    # ``\nabla``, ``\nu``).
    if "\n" not in markdown:
        markdown = markdown.replace("\\n", "\n")
    return {
        "type": "doc",
        "content": _build_nodes(markdown.splitlines()),
    }


def markdown_to_draft_body(markdown: str) -> str:
    return json.dumps(markdown_to_draft_doc(markdown), ensure_ascii=False)
    ...
    for line in lines:
        ...
        if in_fence:
            if _FENCE_CLOSE.match(stripped):
                in_fence = False
                if fence_lang == "datawrapper":
                    nodes.append(
                        _draft_datawrapper_placeholder("\n".join(fence_lines).strip())
                    )
                else:
        ...
        if in_latex:
            if _LATEX_FENCE.match(stripped):
                in_latex = False
                nodes.append(_draft_latex_block("\n".join(latex_lines)))
                latex_lines = []
            else:
        ...
        if match := _LATEX_INLINE.match(stripped):
            flush_para()
            flush_list()
            flush_quote()
            nodes.append(_draft_latex_block(match.group(1).strip()))
            continue
        ...
        if match := _IMAGE_LINE.match(stripped):
            flush_para()
            flush_list()
            flush_quote()
            nodes.append(_draft_image_placeholder(match.group(2), match.group(1)))
            continue
...
def _draft_datawrapper_placeholder(url: str) -> dict[str, Any]:
    return {"type": "_unresolved_datawrapper", "attrs": {"url": url}}


def _draft_image_placeholder(src: str, alt: str) -> dict[str, Any]:
    return {"type": "_unresolved_image", "attrs": {"src": src, "alt": alt}}


def _latex_id() -> str:
    """A 10-character uppercase id, matching Substack's editor-generated ids."""
    return "".join(random.choices(string.ascii_uppercase, k=10))


def _draft_latex_block(expression: str) -> dict[str, Any]:
    return {
        "type": "latex_block",
        "attrs": {"persistentExpression": expression, "id": _latex_id()},
    }
...
def _prosemirror_node_to_md(node: dict[str, Any]) -> str | None:
    ...
    if node_type == "latex_block":
        expression = (node.get("attrs") or {}).get("persistentExpression", "")
        return f"$$\n{expression}\n$$"

    if node_type == "datawrapper":
        url = (node.get("attrs") or {}).get("url", "")
        return f"```datawrapper\n{url}\n```"

    if node_type == "captionedImage":
        image = next(
            (c for c in node.get("content", []) if c.get("type") == "image2"),
            None,
        )
        attrs = (image or {}).get("attrs") or {}
        alt = attrs.get("alt") or ""
        return f"![{alt}]({attrs.get('src', '')})"
