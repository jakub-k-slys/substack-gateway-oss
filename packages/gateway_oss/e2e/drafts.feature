Feature: Drafts endpoints
  ...
  Scenario: Draft pre-publish and AI-detection reads
    Given valid credentials
    When I create a draft with title "e2e scan draft" and body "Hello from e2e scan"
    Then the response status is 201
    Given the created draft ID is saved
    When I fetch the test draft prepublish
    Then the response status is 200
    And the response has field "errors"
    And the response has field "suggestions"
    When I fetch the test draft AI detection
    Then the response status is 200
    And the response has field "fraction_ai"
    When I delete the test draft
    Then the response status is 204
